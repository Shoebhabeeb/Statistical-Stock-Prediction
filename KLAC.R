...
#Plot the data
getSymbols("KLAC", from = "1900-01-01",to="2019-11-30", src = 'yahoo', periodicity = 'monthly')
chartSeries(KLAC)
...
subset<-window(KLAC,start = as.Date("1900-01-01"),end=as.Date("2014-11-30"))
train<-cbind(subset$KLAC.Close-subset$KLAC.Open)
...
data_KLAC<- read.csv('~/Desktop/CMS/project/KLAC.csv')
dataset_KLAC<-cbind(data_KLAC$Close-data_KLAC$Open)
trainmean<- mean(dataset_KLAC)
trainsd<-sd(dataset_KLAC)
train = ifelse(dataset_KLAC > (trainmean + 0.5*trainsd), "H",(ifelse(dataset_KLAC < (trainmean - 0.5*trainsd),"L","M")))
train
dataframehidden_visible<- ifelse (dataset_KLAC > 0,"I","D")
dataset_KLAC<-as.data.frame(dataset_KLAC)
dataset_KLAC$V2<-ifelse (dataset_KLAC$V1 > 0,1,2)
dataset_KLAC$V1 = train
Data_Final = dataset_KLAC[c("V1","V2")]
...
mu = mean(KLAC_log_returns)
sig = sd(KLAC_log_returns)
mc_matrix<-matrix(nrow=12*5,ncol=N)
mc_matrix[1,1]<-as.numeric(KLAC$KLAC.Adjusted[length(KLAC$KLAC.Adjusted),])
for(j in 1:ncol(mc_matrix)){
  mc_matrix[1,j]<-as.numeric(KLAC$KLAC.Adjusted[length(KLAC$KLAC.Adjusted),])
...
final_mat%>%gather("Simulation","Price",2:501)%>%ggplot(aes(x=Month,y=Price,Group=Simulation))+geom_line(alpha=0.2)+labs(title="KLA Corporation Stock (ASML): 500 Monte Carlo Simulations for 5 Years")+theme_bw()
final_mat[48,-1]%>%as.numeric()%>%quantile(probs=c(5, 50,95,99.5, NA)/100,na.rm=TRUE)

#Regime Detection
KLACRets = diff( log( Cl( KLAC ) ) )
returns = as.numeric(KLACRets)
plot(KLACRets)

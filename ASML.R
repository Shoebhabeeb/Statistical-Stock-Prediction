...
library(xts)
#Plot the data
getSymbols("ASML", from = "1900-01-01",to="2019-11-30", src = 'yahoo', periodicity = 'monthly')
chartSeries(ASML)
...
subset<-window(ASML,start = as.Date("1900-01-01"),end=as.Date("2014-11-30"))
train<-cbind(subset$ASML.Close-subset$ASML.Open)
...
data_ASML<- read.csv('~/Desktop/CMS/project/ASML.csv')
dataset_ASML<-cbind(data_ASML$Close-data_ASML$Open)
trainmean<- mean(dataset_ASML)
trainsd<-sd(dataset_ASML)
train = ifelse(dataset_ASML > (trainmean + 0.5*trainsd), "H",(ifelse(dataset_ASML < (trainmean - 0.5*trainsd),"L","M")))
train
dataframehidden_visible<- ifelse (dataset_ASML > 0,"I","D")
dataset_ASML<-as.data.frame(dataset_ASML)
dataset_ASML$V2<-ifelse (dataset_ASML$V1 > 0,1,2)
dataset_ASML$V1 = train
Data_Final = dataset_ASML[c("V1","V2")]
...
N<-500
mu = mean(ASML_log_returns)
sig = sd(ASML_log_returns)
mc_matrix<-matrix(nrow=12*5,ncol=N)
mc_matrix[1,1]<-as.numeric(ASML$ASML.Adjusted[length(ASML$ASML.Adjusted),])
for(j in 1:ncol(mc_matrix)){
  mc_matrix[1,j]<-as.numeric(ASML$ASML.Adjusted[length(ASML$ASML.Adjusted),])
...
dim(final_mat) #60 501
final_mat%>%gather("Simulation","Price",2:501)%>%ggplot(aes(x=Month,y=Price,Group=Simulation))+geom_line(alpha=0.2)+labs(title="ASML Holding Stock (ASML): 500 Monte Carlo Simulations for 5 Years")+theme_bw()
final_mat[48,-1]%>%as.numeric()%>%quantile(probs=c(5, 50,95,99.5, NA)/100,na.rm=TRUE)

#Regime Detection
ASMLRets = diff( log( Cl( ASML ) ) )
returns = as.numeric(ASMLRets)
plot(ASMLRets)
